...
(function() {
  var AndType, Any, HasAnnotation, Nothing, OrType, TermDecl, Token, TypeDecl, WithAnnotation, WithGuard, channels, clearLog, createPredefContext, deepCopyTree, derivedAndOrType, derivedAssignment, derivedConstruction, derivedNameSelect, derivedTermDecl, derivedTypeDecl, doStatementCompleters, doTermCompleters, doTypeCompleters, findMember, findMemberInContext, freshContext, gen, genConstructor, genInitializer, getDefContext, isDefinedInSameContext, isSameReference, isSubType, linearizedForConstruction, log, parse, replaceContextWithContext, replaceContextWithPrefix, requireCompatibility, requireDefContext, requireMemberInContext, requireMemberInType, requireTermCompatibility, shallowCopyTree, simplifyType, stderr, stringifyStatements, tabs, tokenize, typeAsConstructed, typeAsLinearized, types, widen,
  ...
  widen = function(tree, ctx, returnLowerBound, logIndent) {
    var asConstructed, prefixCtx, prefixTyp, ref, ref1, typ;
    if (logIndent == null) {
      logIndent = 0;
    }
    if (tree.type === "id") {
      log(types, tabs(logIndent) + ("Widening " + tree.match + " to its declared type") + (returnLowerBound ? " (getting lower bound)" : ""));
      ...
      log(types, tabs(logIndent) + ("Widening term selection " + (tree.stringify(logIndent + 1)) + " to its declared type") + (returnLowerBound ? " (getting lower bound)" : ""));
      ...
      log(types, "Result: " + typ.stringify(0));
      return [prefixCtx, typ];
    } else if (tree.type === "TYPE-SELECT") {
      log(types, tabs(logIndent) + ("Widening type selection " + (tree.stringify(logIndent + 1)) + " to its declared type") + (returnLowerBound ? " (getting lower bound)" : ""));
  ...
  typeAsLinearized = function(tree, ctx) {
    var base, bases, j, len, problemBase, ref, stmts, typ;
    ref = linearizedForConstruction(tree, ctx), bases = ref[0], problemBase = ref[1], stmts = ref[2];
    if (bases === false) {
      throw new Error("Cannot construct object at line " + tree.line + " character " + tree.column + " because base type " + (problemBase.stringify(0)) + " is non-constructible.");
    }
