...
(function() {
  var AndType, Any, FieldMember, Nothing, OrType, TermSelect, TypeMember, TypeSelect, contextify, findMember, getMembershipOf, lowerBoundMembership, membershipOfStatements, parse, strtyp, tabs, tokenize, typecheck, upperBoundMembership,
  ...
  window.compile = function(input, stopAfter) {
    ...
    try {
      ...
      debugOutput = contextify(null, ast);
  ...
  membershipOfStatements = function(tree) {
    var j, len, members, ref, st, typeMembers;
    members = {
      typeMembers: {},
      termMembers: {}
    };
  ...
  getMembershipOf = function(tree) {
    if (tree.type === "STATEMENTS") {
      return membershipOfStatements(tree);
    }
  };
  ...
  contextify = function(outerContext, tree) {
    ...
    if (tree.type === "STATEMENTS") {
      ...
      tree.context.outerContext = outerContext;
      tree.context.lookupSymbol = function(symName) {
        if (symName in tree.context.symbols) {
          return tree.context.symbols[symName];
        } else if (tree.context.outerContext) {
          return tree.context.outerContext.lookupSymbol(symName);
        } else {
          return null;
        }
      };
    ...
    reduce = function() {
      ...
      if (matches(["ID", "COLON", "*", "ATLEAST", "*", "NEWLINE"])) {
        ...
        if (matches(["TYPE", "ATLEAST", "TYPE"])) {
          ...
          t.stringify = function(indent) {
            return t.lhs.stringify(indent) + ": " + t.rhsUpper.stringify(indent) + strtyp(t, indent);
          };
          t.subtrees = function() {
            return [t.lhs, t.rhs];
          };
  ...
  tokenize = function(input) {
    var Whitespace, column, consumeNextToken, getNextToken, line, t, tokenList, tokens;
    line = 1;
    column = 1;
    tokenList = [
      {
    ...
    getNextToken = function() {
      var j, len, length, matches, tk, tok;
