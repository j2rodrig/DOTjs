...
(function() {
  var Any, Nothing, Token, TypeBounds, channels, clearLog, createGlobalContext, derivedAndOrType, derivedTypeBounds, findMemberInContext, forAllBaseTypesInTypeTree, freshContext, genConstructor, genInitializer, getBaseTypes, linearizeBaseTypes, log, lowerBound, parse, regDeclaredMembers, regTypeAssignments, registerMembers, requireMemberInContext, simplifyType, stderr, tabs, tokenize, traverseTree, upperBound,
  ...
  *There is a canonical form for any type tree: (S1 & ... & SN) | ... | (T1 & ... & TM) where every Sn and Tm is a
  ...
  freshContext = function(outer, tree) {
    ...
    ctx.addSymbol = function(name, typTree, origCtx) {
      var record;
      if (ctx.members[name]) {
  ...
  getBaseTypes = function(typTree, ctx) {
    var b, j, len, lhsBases, ref, rhsBases;
    if ((ref = typTree.type) === "ID" || ref === "TYPE-SELECT") {
      return [typTree];
    } else if (typTree.type === "TYPE-BOUNDS") {
      return getBaseTypes(typTree.lower, ctx);
    } else if (typTree.type === "AND-TYPE") {
      return getBaseTypes(typTree.lhs, ctx).concat(getBaseTypes(typTree.rhs, ctx));
  ...
  registerMembers = function(typTree, ctorCtx, treeCtx) {
    var existing, j, len, ref, results, st;
    if (typTree === "STATEMENTS") {
      ref = typTree.statements;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        st = ref[j];
        if (st === "TYPE-DECL") {
          if (!ctorCtx.members[st.lhs.match]) {
            results.push(ctorCtx.addSymbol(st.lhs.match, TypeBounds(st.rhsLower, st.rhsUpper), treeCtx));
          } else {
            existing = ctorCtx.members[st.lhs.match];
            results.push(existing.typTree = TypeBounds(simplifyType(OrType(st.rhsLower, existing.typTree.lower)), simplifyType(AndType(st.rhsUpper, existing.typTree.upper))));
          }
        } else if (st === "TERM-DECL") {
          if (!ctorCtx.members[st.lhs.match]) {
            results.push(ctorCtx.addSymbol(st.lhs.match, st.rhs, treeCtx));
          } else {
            results.push(existing = ctorCtx.members[st.lhs.match]);
          }
        } else {
          results.push(void 0);
        }
  ...
  linearizeBaseTypes = function(_typTree, _ctx) {
    var baseTypes, linearize;
    baseTypes = [];
    linearize = function(typTree, ctx) {
      var lhsContainsNothing, lhsTrees, ref, rhsTrees;
      if ((ref = typTree.type) === "ID" || ref === "STATEMENTS" || ref === "TYPE-SELECT") {
        if (!(indexOf.call(baseTypes, typTree) >= 0)) {
          return baseTypes.push(typTree);
        }
      } else if (typTree.type === "AND-TYPE") {
        linearize(typTree.lhs, ctx);
        return linearize(typTree.rhs, ctx);
      } else if (typTree.type === "OR-TYPE") {
        lhsTrees = linearizeBaseTypes(typTree.lhs, ctx);
        lhsContainsNothing = false;
        return rhsTrees = linearizeBaseTypes(typTree.rhs, ctx);
      } else if (typTree.type === "STATEMENTS") {
        if (!(indexOf.call(_baseClasses, typTree) >= 0)) {
          return _baseClasses.push(typTree);
        }
      }
    };
    linearize(_typTree, _ctx);
    return baseTypes;
  };

  traverseTree = function(tree, fn) {
    var j, len, r, ref, results, st;
    r = fn(tree);
    if (r && (tree.subtrees != null)) {
      ref = tree.subtrees();
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        st = ref[j];
        results.push(traverseTree(st, fn));
      }
      return results;
    }
  };

  forAllBaseTypesInTypeTree = function(tree, fn) {
    var ref;
    if ((ref = tree.type) === "ID" || ref === "STATEMENTS" || ref === "TYPE-SELECT") {
      return fn(tree);
    } else if (tree.type === "AND-TYPE") {
      forAllBaseTypesInTypeTree(tree.lhs, fn);
      return forAllBaseTypesInTypeTree(tree.rhs, fn);
    } else if (tree.type === "OR-TYPE") {
      if (tree.lhs.type === "ID" && tree.lhs.match === "Nothing") {
        return forAllBaseTypesInTypeTree(tree.rhs);
      } else {
        return forAllBaseTypesInTypeTree(tree.lhs);
      }
    } else {
      throw new Error("Internal compiler error : Expected a type tree in forAllBaseTypesInTypeTree, got " + tree.type + " tree");
    }
  };
  ...
  requireMemberInContext = function(name, ctx, tree) {
    var record;
    record = findMemberInContext(name, ctx);
    if (!record) {
  ...
  regDeclaredMembers = function(ctx) {
    return forAllBaseTypesInTypeTree(ctx.tree, function(tree) {
      var definitionContext;
      if (tree.type === "ID") {
        return definitionContext = requireMemberInContext(tree.match, ctx.outer).typTree.ctx;
      }
    });
  };

  regTypeAssignments = function(ctx) {};
  ...
  genConstructor = function(tree, ctx, indent) {
    var superInits;
    ctx = ctx.fresh(tree);
  ...
  genInitializer = function(tree, ctx, indent) {
    var stmts;
    ctx = ctx.fresh(tree);
    regDeclaredMembers(ctx);
