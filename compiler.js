...
(function() {
  var Any, Nothing, Token, TypeBounds, createGlobalContext, derivedAndOrType, derivedTypeBounds, enclosingConstructorContext, findMemberInContext, findMemberInTypeTree, findNamedMembers, freshContext, genConstructor, genMixinConstructor, genStatementsFromTypeTree, genTerm, lowerBound, parse, registerSymbols, simplifyType, tabs, tokenize, upperBound, widen,
  ...
  window.compile = function(input, stopAfter) {
    ...
    try {
      ...
      output = genConstructor(ast, ast.ctx, 0);
      return output;
    } catch (error1) {
      error = error1;
      if (error.stack) {
  ...
  *There is a canonical form for any type tree: (S1 & ... & SN) | ... | (T1 & ... & TM) where every Sn and Tm is a
  ...
  findMemberInContext = function(name, ctx) {
    ...
    typTree = ctx.isConstructor || ctx.isMixinConstructor ? (console.log("findMemberInContext '" + name + "' in constructor context '" + ctx.name + "'"), findMemberInTypeTree(name, ctx.tree)) : ctx.members[name] ? (console.log("findMemberInContext '" + name + "' in context '" + ctx.name + "' : Found"), ctx.members[name].typTree) : void 0;
    if (typTree) {
      return typTree;
    } else {
  ...
  findMemberInTypeTree = function(name, tree, useLowerBound) {
    var lhs, rhs, wt;
    if (!tree) {
    ...
    if (tree.type === "STATEMENTS") {
      ...
      return findMemberInTypeTree(name, wt, useLowerBound);
    } else if (tree.type === "ID") {
      wt = widen(tree);
      if (!wt) {
        throw "Error: Cannot find definition of '" + tree.match + "' on line " + tree.line + " character " + tree.column;
      ...
      if (useLowerBound) {
        return findMemberInTypeTree(name, tree.lower, useLowerBound);
      } else {
        return findMemberInTypeTree(name, tree.upper, useLowerBound);
      }
    } else if (tree.type === "AND-TYPE") {
  ...
  widen = function(tree) {
    ...
    if (tree.type === "TYPE-SELECT") {
      ...
      return findMemberInContext(tree.match, tree.ctx);
    } else {
  ...
  genStatementsFromTypeTree = function(tree, ctorCtx, indent) {
    var assignto, ctor, js, jsLhs, jsRhs, prefixJs, prefixTypTree, ref, ref1, ref2, ref3, ref4, st, typLhs, typRhs, typTree;
    if (tree.type === "STATEMENTS") {
      return ((function() {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          st = ref[j];
          results.push(genStatementsFromTypeTree(st, ctorCtx, indent + 1));
        }
        return results;
      })()).join('');
    } else if (tree.type === "TYPE-DECL") {
      ...
      if (typTree) {
        return tabs(indent) + ((enclosingConstructorContext(typTree.ctx).name) + "." + tree.ID + "();\n");
      } else {
        throw "Error : Cannot find definition of '" + tree.ID + "' on line " + tree.line + " character " + tree.column;
      }
  ...
  genTerm = function(tree, ctx, indent) {
    var prefixJs, prefixTypTree, ref, typTree;
    if (tree.type === "id") {
      typTree = findMemberInContext(tree.match, tree.ctx);
      if (typTree) {
        return [(enclosingConstructorContext(typTree.ctx).name) + "." + tree.match, typTree];
      } else {
        throw "Error : Cannot find definition of '" + tree.match + "' on line " + tree.line + " character " + tree.column;
      }
    } else if (tree.type === "TERM-SELECT") {
      ref = genTerm(tree, ctx, indent), prefixJs = ref[0], prefixTypTree = ref[1];
      typTree = findMemberInTypeTree(tree.id, prefixTypTree);
      if (typTree) {
        return [prefixJs + "." + tree.id, typTree];
      } else {
        throw "Error : Cannot find definition of '" + tree.id + "' on line " + tree.line + " character " + tree.column;
      }
    } else if (tree.type === "CONSTRUCT") {
