...
(function() {
  var Any, Nothing, Token, TypeBounds, channels, clearLog, contextTypeString, createGlobalContext, derivedAndOrType, derivedTypeBounds, enclosingConstructorContext, findMemberInContext, findMemberInTypeTree, findNamedMembers, freshContext, genConstructor, genMixinConstructor, genStatementsFromTypeTree, genTerm, log, lowerBound, parse, registerSymbols, simplifyType, stderr, tabs, tokenize, upperBound, widen,
    indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  window.compile = function(input, stopAfter) {
    var ast, error, error1, globalCtx, logText, message, output, programInstantiateCtx, stacktrace, tok, tokens;
    try {
      clearLog();
      ...
      programInstantiateCtx.isConstructor = true;
  ...
  *There is a canonical form for any type tree: (S1 & ... & SN) | ... | (T1 & ... & TM) where every Sn and Tm is a
  ...
  createGlobalContext = function(tree) {
    var ctx;
  ...
  freshContext = function(outer, tree) {
    var ctx;
    ctx = {};
    if (outer) {
    ...
    ctx.members = {};
    ctx.fresh = function(tr) {
      return freshContext(ctx, tr);
    };
    ctx.addSymbol = function(name, typTree, meta) {
      var record;
      if (ctx.members[name]) {
  ...
  enclosingConstructorContext = function(ctx) {
    if (ctx.isConstructor || ctx.isMixinConstructor) {
      return ctx;
    } else if (ctx.outer) {
      return enclosingConstructorContext(ctx.outer);
    } else {
      return ctx;
    }
  };

  contextTypeString = function(ctx) {
    var s;
    s = [];
    if (ctx.isConstructor) {
      s.push("constructor");
    }
    if (ctx.isMixinConstructor) {
  ...
  registerSymbols = function(tree, ctx) {
    var j, len, ref, results, subtree, typTree;
    tree.ctx = ctx;
    ...
    if (tree.subtrees) {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        subtree = ref[j];
        results.push(registerSymbols(subtree, ctx));
      }
      return results;
    }
  };
  ...
  findNamedMembers = function(tree, typesOnly) {
    var both, j, k, l, len, len1, len2, lhs, members, name, ref, rhs, st, wt;
    if (tree.type === "STATEMENTS") {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        } else if (tree.type === "TERM-DECL" && !typesOnly) {
          members.push(st.lhs.match);
        }
      }
      return members;
      ...
      return findNamedMembers(wt, typesOnly);
      ...
      return findNamedMembers(wt, typesOnly);
    } else if (tree.type === "TYPE-BOUNDS") {
      return findNamedMembers(tree.lower, typesOnly);
    } else if (tree.type === "CONSTRUCT") {
      return findNamedMembers(tree.typTree, typesOnly);
    } else if (tree.type === "AND-TYPE") {
      ...
      if (lhs === Nothing || rhs === Nothing) {
        ...
        for (k = 0, len1 = rhs.length; k < len1; k++) {
          name = rhs[k];
          if (!(indexOf.call(lhs, name) >= 0)) {
            lhs.push(name);
          }
        }
        return lhs;
      ...
      if (lhs === Nothing) {
        ...
        both = [];
        for (l = 0, len2 = lhs.length; l < len2; l++) {
          name = lhs[l];
          if (indexOf.call(rhs, name) >= 0) {
            both.push(name);
          }
        }
        return both;
  ...
  findMemberInContext = function(name, ctx, seen) {
    ...
    if (record) {
      ...
      return findMemberInContext(name, ctx.outer, seen);
    }
  };

  findMemberInTypeTree = function(name, tree, useLowerBound, seen) {
    var lhs, rhs, wt;
    if (tree && tree.type) {
    ...
    if (!seen) {
      ...
      seen[name] = [tree];
    } else if (indexOf.call(seen[name], tree) >= 0) {
    ...
    if (tree.type === "STATEMENTS") {
      ...
      return findMemberInTypeTree(name, wt, useLowerBound, seen);
    } else if (tree.type === "ID") {
      ...
      if (useLowerBound) {
        return findMemberInTypeTree(name, tree.lower, useLowerBound, seen);
      } else {
        return findMemberInTypeTree(name, tree.upper, useLowerBound, seen);
      }
    } else if (tree.type === "AND-TYPE") {
      lhs = findMemberInTypeTree(name, tree.lhs, useLowerBound, seen);
      rhs = findMemberInTypeTree(name, tree.rhs, useLowerBound, seen);
      if (lhs && rhs) {
        return simplifyType(TypeBounds(OrType(lowerBound(lhs), lowerBound(rhs)), AndType(upperBound(lhs), upperBound(rhs))));
      } else if (lhs) {
        return lhs;
      } else if (rhs) {
        return rhs;
      ...
      rhs = findMemberInTypeTree(name, tree.rhs, useLowerBound, seen);
      if (lhs && rhs) {
        return simplifyType(TypeBounds(AndType(lowerBound(lhs), lowerBound(rhs)), OrType(upperBound(lhs), upperBound(rhs))));
      } else {
        return void 0;
      }
    } else {
  ...
  widen = function(tree, seen) {
    var prefixTypTree, record, ref;
    if (tree.type === "TYPE-SELECT") {
      prefixTypTree = widen(tree.prefix, seen);
      log(stderr, "Finding " + tree.ID + " in widened type");
      return findMemberInTypeTree(tree.ID, prefixTypTree, seen);
    } else if (tree.type === "TERM-SELECT") {
      prefixTypTree = widen(tree.prefix, seen);
      log(stderr, "Finding " + tree.id + " in widened type");
      return findMemberInTypeTree(tree.id, prefixTypTree, seen);
    } else if ((ref = tree.type) === "ID" || ref === "id") {
  ...
  genConstructor = function(typTree, ctorCtx, indent) {
    return "(" + genMixinConstructor(typTree, ctorCtx, indent) + ")({})";
  };

  genMixinConstructor = function(typTree, ctorCtx, indent) {
    var postfix, preamble, statements;
    preamble = "function(" + ctorCtx.name + "){\n";
    statements = genStatementsFromTypeTree(typTree, ctorCtx, indent + 1);
    postfix = tabs(indent) + "}";
    return preamble + statements + postfix;
  };

  genStatementsFromTypeTree = function(tree, ctorCtx, indent) {
    var assignto, ctor, js, jsLhs, jsRhs, origCtx, prefixJs, prefixTypTree, record, ref, ref1, ref2, ref3, ref4, st, typLhs, typRhs, typTree;
    if (tree.type === "STATEMENTS") {
      return ((function() {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          st = ref[j];
          results.push(genStatementsFromTypeTree(st, ctorCtx, indent));
        }
        return results;
      })()).join('');
    } else if (tree.type === "TYPE-DECL") {
      ...
      return tabs(indent) + assignto + ctor + ";\n";
    } else if (tree.type === "TERM-DECL") {
      return '';
    } else if (tree.type === "TYPE-SELECT") {
      ref = genTerm(tree.prefix, ctorCtx, indent + 1), prefixJs = ref[0], prefixTypTree = ref[1];
      record = findMemberInTypeTree(tree.ID, prefixTypTree);
      if (record) {
        return tabs(indent) + (prefixJs + "." + tree.ID + "(ctorCtx.name);\n");
      } else {
        throw "Error : Cannot find definition of '" + tree.id + "' on line " + tree.line + " character " + tree.column;
      }
    } else if (tree.type === "ID") {
      ...
      if (record) {
        ...
        return tabs(indent) + (origCtx.name + "." + tree.match + "(" + ctorCtx.name + ");\n");
      } else {
        throw "Error : Cannot find definition of '" + tree.ID + "' on line " + tree.line + " character " + tree.column;
      }
    } else if (tree.type === "TERM-ASSIGN") {
      ref1 = genTerm(tree.lhs, ctorCtx, indent), jsLhs = ref1[0], typLhs = ref1[1];
      ref2 = genTerm(tree.rhs, ctorCtx, indent), jsRhs = ref2[0], typRhs = ref2[1];
      return tabs(indent) + jsLhs + " = " + jsRhs + ";\n";
    } else if ((ref3 = tree.type) === "id" || ref3 === "TERM-SELECT" || ref3 === "CONSTRUCT") {
      ref4 = genTerm(tree, ctorCtx, indent), js = ref4[0], typTree = ref4[1];
      return tabs(indent) + js + ";\n";
    } else {
      throw "Internal compiler error: Unimplemented tree type '" + tree.type + "' in genStatementsFromTypeTree";
    }
  };

  genTerm = function(tree, ctx, indent) {
    var origCtx, prefixJs, prefixTypTree, record, ref;
    if (tree.type === "id") {
      log(stderr, "Starting code generation for id " + tree.match + " in " + ctx.name);
      record = findMemberInContext(tree.match, ctx);
      if (record) {
        origCtx = enclosingConstructorContext(record.ctx);
        return [origCtx.name + "." + tree.match, record.typTree];
      } else {
        throw "Error : Cannot find definition of '" + tree.match + "' on line " + tree.line + " character " + tree.column;
      }
    } else if (tree.type === "TERM-SELECT") {
      log(stderr, "Starting code generation for term selection of " + tree.id);
      ref = genTerm(tree.prefix, ctx, indent), prefixJs = ref[0], prefixTypTree = ref[1];
      record = findMemberInTypeTree(tree.id, prefixTypTree);
      if (record) {
        return [prefixJs + "." + tree.id, record.typTree];
      } else {
        throw "Error : Cannot find definition of '" + tree.id + "' on line " + tree.line + " character " + tree.column;
      }
    } else if (tree.type === "CONSTRUCT") {
      return [genConstructor(tree.typTree, tree.ctx, indent), tree.typTree];
    } else if (tree.type === "NOT-IMPLEMENTED") {
      return ['(function(){throw "Not Implemented"})()', Nothing];
    } else {
