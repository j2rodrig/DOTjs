...
(function() {
  var AndType, Any, Nothing, OrType, TakeLowerBound, TermDecl, Token, TypeBounds, TypeDecl, channels, clearLog, createPredefContext, derivedAndOrType, derivedTypeBounds, doStatementCompleters, doTermCompleters, doTypeCompleters, findBaseStatementBlocks, findMember, findMemberInContext, freshContext, gen, genConstructor, genInitializer, getBaseTypes, getDefContext, log, lowerBound, parse, requireDefContext, requireMemberInContext, requireMemberInType, simplifyType, stderr, tabs, tokenize, typeAsConstructed, upperBound, widen,
    indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

  window.compile = function(input, stopAfter) {
    var ast, error, error1, logText, message, output, outputBuffer, predefCtx, stacktrace, tok, tokens;
    try {
      clearLog();
  ...
  *There is a canonical form for any type tree: (S1 & ... & SN) | ... | (T1 & ... & TM) where every Sn and Tm is a
  ...
  TakeLowerBound = function(underlying) {
    return {
  ...
  freshContext = function(outer, typTree) {
    ...
    ctx.findMember = function(name) {
      ...
      return findMember(name, asConstructed, ctx.outer, true);
    };
    return ctx;
  };

  findMember = function(name, typTree, ctx, returnLowerBound) {
    ...
    if (typTree.type === "STATEMENTS") {
      ...
      return findMember(name, widenedIdTree, widenedIdTree.ctx, returnLowerBound);
    } else if (typTree.type === "TYPE-SELECT") {
      widenedTyp = widen(typTree, ctx);
      return findMember(name, widenedTyp, widenedTyp.ctx, returnLowerBound);
  ...
  widen = function(tree, ctx, returnLowerBound) {
    var prefixTyp;
    if (tree.type === "id") {
      return requireMemberInContext(tree.match, ctx, tree);
    } else if (tree.type === "ID") {
      return requireMemberInContext(tree.match, ctx, tree, returnLowerBound);
      ...
      prefixTyp = widen(tree.prefix, ctx);
      return requireMemberInType(tree.ID, prefixTyp, tree, returnLowerBound);
    } else if (tree.type === "CONSTRUCT") {
      return typeAsConstructed(tree.typTree, ctx);
    } else {
  ...
  requireMemberInContext = function(name, ctx, sourceTree) {
    var typTree;
  ...
  requireMemberInType = function(name, typTree, sourceTree, returnLowerBound) {
    var found;
  ...
  findBaseStatementBlocks = function(typTree, ctx) {
    var base, basesSeen, j, k, len, len1, ref, ref1, ref2, statementsFound, stmts, widenedType;
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (!(indexOf.call(basesSeen, base) >= 0)) {
        ...
        } else if ((ref1 = base.type) === "ID" || ref1 === "TYPE-SELECT") {
          widenedType = widen(base, ctx, true);
          ref2 = findBaseStatementBlocks(widenedType, widenedType.ctx.outer);
  ...
  doTermCompleters = function(tree, ctx) {
    if (tree.type === "id") {
      ...
      tree.info = function() {
        return requireMemberInType(tree.id, tree.prefix.info(), tree);
      };
      return doTermCompleters(tree.prefix, ctx);
    } else if (tree.type === "CONSTRUCT") {
  ...
  parse = function(tokens) {
    ...
    reduce = function() {
      ...
      /* Handle Comments */
      var ID, _typTree, a, b, construct, id, lhs, lparen, nw, prefix, rhs, rhsLower, rhsUpper, stmt, stmts, t;
      if (matches(["COMMENT"])) {
        stack.pop();
        return true;
      }
